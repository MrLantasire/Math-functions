...
float Get_direction_from_geopoints(float first_latitude, float first_longitude, float second_latitude, float second_longitude)
{
    ...
    argument = cos( GRAD_TO_RAD(first_latitude) ) * sin( GRAD_TO_RAD(second_latitude) ) 
    - sin( GRAD_TO_RAD(first_latitude) ) * cos( GRAD_TO_RAD(second_latitude) ) * cos( GRAD_TO_RAD(first_longitude - second_longitude) );

    return ((float) RAD_TO_GRAD(atan2(sin(GRAD_TO_RAD(second_longitude - first_longitude)) * cos(second_latitude), argument)));
}
